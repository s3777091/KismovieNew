...
const getListHomePage = async (url) => {
  ...
  for (let i = listMV.length - 1; i > 0; i--) {
    let linkClones = await MovieModel.getListLinkByCloneFrom(hostClone.WEBPHIMHAY);
...
const getInfoPhim = async (linkMV) => {
  ...
  let infoMovie = {
    ...
    cloneFrom: hostClone.WEBPHIMHAY,
  };
  let movieOption = {
    times,
    category: category,
...
const rebaseLinkClone = (link) => {
  return link.replace(hostClone.BASE_LINK_WEBPHIMHAY, hostClone.WEBPHIMHAY);
};
